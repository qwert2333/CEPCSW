...
//    else                                                   m_datacol.GoodClus3DCol[ic].setType(2);
  ...
  std::vector<CRDEcalEDM::CRDCaloHit2DShower> m_MIPshowerCol; m_MIPshowerCol.clear();
  std::vector<CRDEcalEDM::CRDCaloHit2DShower> m_EMshowerCol;  m_EMshowerCol.clear(); 

  for(int ic=0; ic<m_datacol.GoodClus3DCol.size(); ic++){
    if(m_datacol.GoodClus3DCol[ic].getType()==0){
      std::vector<CRDEcalEDM::CRDCaloHit2DShower> tmp_col = m_datacol.GoodClus3DCol[ic].get2DShowers(); 
  ...
  for(int ic=0; ic<m_datacol.BadClus3DCol.size(); ic++){
    std::vector<CRDEcalEDM::CRDCaloHit2DShower> tmp_col = m_datacol.BadClus3DCol[ic].get2DShowers(); 
    m_showerCol.insert( m_showerCol.end(), tmp_col.begin(), tmp_col.end() );
  }
