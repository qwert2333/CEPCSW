...
StatusCode ConeClusteringAlg::ReadSettings(Settings& m_settings){
  ...
  if(settings.map_stringPars.find("ReadinHit")==settings.map_stringPars.end())        settings.map_stringPars["ReadinHit"] = "EcalShowerInLayer";
  ...
  if(settings.map_floatPars.find("th_ConeR_l1")==settings.map_floatPars.end())        settings.map_floatPars["th_ConeR_l1"] = 100.;
  ...
  if(settings.map_floatPars.find("th_ConeR_l2")==settings.map_floatPars.end())        settings.map_floatPars["th_ConeR_l2"] = 150.;
...
cout<<"  ConeClustering: Readin "<<settings.map_stringPars["ReadinHit"]<<", Output: "<<settings.map_stringPars["OutputCluster"]<<endl;
  ...
  //Get readin info
  if(settings.map_stringPars["ReadinHit"] == "EcalShowerInLayer"){
...
//cout<<"     Cluster size: "<<ClusterCol.size()<<endl;
      ...
      for(int ic=0; ic<ClusterCol.size(); ic++ ){
        ...
        TVector3 axis = ClusterCol[ic]->getAxis(); 
...
//    axis.x(), axis.y(), axis.z() );
        ...
        if(  (m_Nhits<3 && m_Nhits>0 && relR_vec.Angle(axis) < settings.map_floatPars["th_ConeTheta_l1"] && relR_vec.Mag()< settings.map_floatPars["th_ConeR_l1"]) ||
             (m_Nhits>=3             && relR_vec.Angle(axis) < settings.map_floatPars["th_ConeTheta_l2"] && relR_vec.Mag()< settings.map_floatPars["th_ConeR_l2"])  ){

          ClusterCol[ic]->addHit(m_hit);
          ClusterCol[ic]->FitAxisHit();
...
PandoraPlus::Calo3DCluster* ConeClusteringAlg::GetClosestGoodCluster( std::vector< PandoraPlus::Calo3DCluster* >& m_goodClusCol, PandoraPlus::Calo3DCluster* m_badClus ){
  ...
  TVector3 m_clusCent = m_badClus->getHitCenter();
  PandoraPlus::Calo3DCluster* m_clus = nullptr;
  double minTheta=999;
  for(int i=0;i<m_goodClusCol.size();i++){
     TVector3 vec_goodClus = m_goodClusCol[i]->getHitCenter();
