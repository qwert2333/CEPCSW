...
//	const edm4hep::SimMCParticleG4* SimMC =  r_MCParG4.get();
...
/*        info() << p.getObjectID().index << " : [";
	    ...
	    const edm4hep::Vector3d& endpoint = p.getEndpoint();
     	m_MCmomentum_x[m_nParticles] = momentum.x;
    	m_MCmomentum_y[m_nParticles] = momentum.y;
      	m_MCmomentum_z[m_nParticles] = momentum.z;
	m_MCendpoint_x[m_nParticles] = endpoint.x;
	m_MCendpoint_y[m_nParticles] = endpoint.y;
  	m_MCendpoint_z[m_nParticles] = endpoint.z;
        m_nParticles++;
	}
        info() << " ]; ";
*/
