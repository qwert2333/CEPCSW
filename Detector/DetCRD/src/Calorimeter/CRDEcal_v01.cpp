...
static dd4hep::Ref_t create_detector(dd4hep::Detector& theDetector,
	...
	int Nblock_z   = 10;							//block number in z direction
	...
	for(int ilayer=1; ilayer<=Nlayers; ilayer++){
		...
		for(int iphi=1; iphi<=Nblock_phi; iphi++){
			for(int iz=1; iz<=Nblock_z; iz++){
				...
				block.setVisAttributes(theDetector, "InvisibleWithChildren");
				...
				plv.addPhysVolID("dlayer", ilayer).addPhysVolID("part", iphi).addPhysVolID("block",iz);
				sd.setPlacement(plv);
